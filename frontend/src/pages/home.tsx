...
export default function home(){
  ...
  const handleVideoSubmit = async (url: string, summaryLength: string) => {
    try {
      ...
      const descriptionNodes = Object.keys(data.mind_map).flatMap((key: string, index: number) =>
        data.mind_map[key].map((desc: string, descIndex: number) => ({
      ...
      const descriptionLinks = Object.keys(data.mind_map).flatMap((key: string, index: number) =>
        data.mind_map[key].map((_: string, descIndex: number) => ({
          source: `node-${index}`,
